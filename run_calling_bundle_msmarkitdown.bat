...
SET "TEXTFILE_A=filea.ext"
SET "TEXTFILE_B=fileb.ext"
...
python dist/mdmtoolsap_bundle.py --program read_msmarkitdown --inpfile "%TEXTFILE_A%"
...
python dist/mdmtoolsap_bundle.py --program read_msmarkitdown --inpfile "%TEXTFILE_B%"
if %ERRORLEVEL% NEQ 0 ( echo ERROR: Failure && pause && goto CLEANUP && exit /b %errorlevel% )
