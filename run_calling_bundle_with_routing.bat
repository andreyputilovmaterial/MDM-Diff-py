...
ECHO write to: .json
python dist/mdmtoolsap_bundle.py --program read --mdd "%MDD_A%" --config-features label,properties --config-section mdmproperties,languages,shared_lists,fields,pages,routing
...
ECHO write to: .json
python dist/mdmtoolsap_bundle.py --program read --mdd "%MDD_B%" --config-features label,properties --config-section mdmproperties,languages,shared_lists,fields,pages,routing
if %ERRORLEVEL% NEQ 0 ( echo ERROR: Failure && pause && goto CLEANUP && exit /b %errorlevel% )
