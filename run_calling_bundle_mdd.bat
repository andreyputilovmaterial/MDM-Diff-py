...
FOR /F "delims=" %%i IN ('python dist/mdmtoolsap_bundle.py --program diff --cmp-scheme-left "%MDD_A_JSON%" --cmp-scheme-right "%MDD_B_JSON%" --norun-special-onlyprintoutputfilename') DO SET "OUTPUT_FINAL_DIFF_JSON=%%i"
...
ECHO -
ECHO 5. diff
python dist/mdmtoolsap_bundle.py --program diff --cmp-scheme-left "%MDD_A_JSON%" --cmp-scheme-right "%MDD_B_JSON%" --output-filename "!OUTPUT_FINAL_DIFF_JSON!" --config-casesensitive-item-list-comparison ignorecase
if !ERRORLEVEL! NEQ 0 ( echo ERROR: Failure && pause && goto CLEANUP && exit /b !ERRORLEVEL! )
