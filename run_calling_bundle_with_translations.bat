...
ECHO write to: .json
python dist/mdmtoolsap_bundle.py --program read --mdd "%MDD_A%" --config-features label,properties,translations --config-section mdmproperties,languages,shared_lists,fields
...
ECHO write to: .json
python dist/mdmtoolsap_bundle.py --program read --mdd "%MDD_B%" --config-features label,properties,translations --config-section mdmproperties,languages,shared_lists,fields
if %ERRORLEVEL% NEQ 0 ( echo ERROR: Failure && pause && goto CLEANUP && exit /b %errorlevel% )
