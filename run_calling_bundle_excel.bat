...
SET "TEXTFILE_A=prepdatadms.xlsx"
SET "TEXTFILE_B=prepdatadms.xlsx"
...
python dist/mdmtoolsap_bundle.py --program read_excel --inpfile "%TEXTFILE_A%"
...
python dist/mdmtoolsap_bundle.py --program read_excel --inpfile "%TEXTFILE_B%"
if %ERRORLEVEL% NEQ 0 ( echo ERROR: Failure && pause && goto CLEANUP && exit /b %errorlevel% )
