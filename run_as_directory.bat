...
ECHO write to: .json
python src\run_universal.py --program read_mdd --mdd "%MDD_A%" --config-features label,attributes,properties --config-section languages,shared_lists,fields
...
ECHO write to: .json
python src\run_universal.py --program read_mdd --mdd "%MDD_B%" --config-features label,attributes,properties --config-section languages,shared_lists,fields
if %ERRORLEVEL% NEQ 0 ( echo ERROR: Failure && pause && exit /b %errorlevel% )
